...
# Most common Markdown Features
This topic is meant to give you a very basic overview of how Markdown works, showing some of the most frequently used operations. Let's start with the most commonly used things:
...
Lets take a closer look at some of the features in detail.
...
Standalone links in Markdown text by default are turned into links:

```markdown
Go to: https://west-wind.com
```
renders autoexpanded as a clickable url:
...
### Images
Images are similar to links using `![alt-text](imageLink)` syntax:
...
### Fontawesome Icons
Markdown Monster includes [custom syntax for FontAwesome icons](https://markdownmonster.west-wind.com/docs/Embedding-Links-Images-Tables-and-More/Embedding-FontAwesome-Icons.html) in its templates. You can embed a `@ icon-` followed by a font-awesome icon name to automatically embed that icon without full HTML syntax.
...
The custom shortcut syntax allows for font-styles, spin and color:

```markdown
@ icon-duotone-gear-color:goldenrod-spin working...
```
